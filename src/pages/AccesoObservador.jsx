...
function AccesoObservador({ token: tokenFromProps }) {
  // Soporta: /accesoObservador/:token o ?token=externo-1JCNP
  const { token: tokenFromRoute } = useParams();
  const [search] = useSearchParams();
  const tokenFromQuery = search.get('token');

  // Priorizamos props > query > route
  let token = tokenFromProps || tokenFromQuery || tokenFromRoute;

  // Convertimos "-" a "/" si es token externo
  let tokenBackend = token?.startsWith('externo-')
    ? token.replace('externo-', 'externo/')
    : token;
  ...
  // ---- Llamada inicial ----
  useEffect(() => {
    if (tokenBackend && !hasCalled.current) {
      registrarAcceso(tokenBackend);
